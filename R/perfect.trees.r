...
mpt_pf <- mpt(formula=eval(parse(text=formultex)), data=salida, start=c(0.7,0.5,rep(0.5,length(study))))
 resultado<-summary(mpt_pf)
 rownames(resultado$coefficients)[1]<-paste0("Prevalence_",study[1])
 for (i in 4:(length(study)+2)){
   rownames(resultado$coefficients)[i]<-paste0("Prevalence_",study[i-2])  
}
 output<-resultado
