...
for (i in 1:length(study)){
     ...
     formultexO[i]<-paste0("p",i,"*sy*(1-sx)+(1-p",i,")*(1-ey)*ex,")
     formultexF[i]<-paste0("p",i,"*(1-sy)*sx+(1-p",i,")*ey*(1-ex),")
     formultexR[i]<-paste0("p",i,"*(1-sy)*(1-sx)+(1-p",i,")*ey*ex,")
}

     formultexR[length(study)]<-paste0("p",i,"*(1-sy)*(1-sx)+(1-p",i,")*ey*ex)")

  formultex<-"freq~list("

for (i in 1:length(study)){
     formultex<-paste0(formultex,formultexA[i],formultexO[i],formultexF[i],formultexR[i])
}
...
mpt222 <- mpt(formula=eval(parse(text=formultex)), data=salida, start=c(0.7,0.5,0.5,0.5,rep(0.5,length(study))))

 results<-summary(mpt222)
 coeff<-results$coefficients
 for (i in 1:dim(coeff)[1]){       
       eval(parse(text=paste0(rownames(coeff)[i],"<-",coeff[i,1])))       
        }
 if ((sy<0.5)&&(sx<0.5)&&(ex<0.5)&&(ey<0.5)){
   for (i in 1:(dim(coeff)[1]-4)) {eval(parse(text=paste0("Pvl",i,"<-1-p",i)))}
   Seny<-1-ey
   Senx<-1-ex
   Espy<-1-sy
   Espx<-1-sx
       ...
       eval(parse(text=paste0("output$Prevl[",i,"]<-Pvl",i)))  
   }
       output$study<-study
}
 else {output<-summary(mpt222)
       output$Seny<-sy
       output$Senx<-sx
       output$Espy<-ey
       output$Espx<-ex 
       for (i in 1:(dim(coeff)[1]-4)){       
       eval(parse(text=paste0("output$Prevl[",i,"]<-p",i)))       
        }
       output$study<-study
}
